...
int background_functions(
  ...
  if (pba->has_nudm == _TRUE_) {
    pvecback[pba->index_bg_rho_nudm] = pba->Omega0_nudm * pow(pba->H0,2) / pow(a,3);
    rho_tot += pvecback[pba->index_bg_rho_nudm];
    //printf("pba->Omega0_nudm=%g\n",pba->Omega0_nudm);
    p_tot += 0.;
  ...
  /* pass value of rho˙cdm2 to output*/
    pvecback[pba->index_bg_rho_cdm2]=pvecback_B[pba->index_bi_rho_cdm2];
    rho_tot += pvecback[pba->index_bg_rho_cdm2];
    p_tot += 0;
    rho_m += pvecback[pba->index_bg_rho_cdm2];
  }
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++) {
      ...
      if (pba->has_nudm == _TRUE_) {
        pvecback[pba->index_bg_A_nudm1+n_ncdm] = pba->u_ncdmdm[n_ncdm]*_sigma_*_Mpc_over_m_*9.47e5* pvecback[pba->index_bg_rho_nudm]/a;
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_cdm2,pba->has_cdm2,index_bg,1);
  ...
  /* -> energy density in cdm2 */
  class_define_index(pba->index_bi_rho_cdm2,pba->has_cdm2,index_bi,1);
...
int background_solve(
  ...
  if (pba->background_verbose > 2) {
    if ((pba->has_dcdm == _TRUE_)&&(pba->has_dr == _TRUE_)) {
      printf("    Decaying Cold Dark Matter details: (DCDM --> DR+CDM2)\n");
      printf("     -> Omega0_dcdm = %f\n",pba->Omega0_dcdm);
      printf("     -> Omega0_dr = %f\n",pba->Omega0_dr);
      printf("     -> Omega0_cdm2 = %f\n",pba->Omega0_cdm2);
      printf("     -> Omega0_dcdm+Omega0_dr+Omega0_cdm2 = %f, input value = %f\n",
             pba->Omega0_dcdm+pba->Omega0_dr+pba->Omega0_cdm2,pba->Omega0_dcdmdrcdm);
  ...
  if (pba->has_cdm2 == _TRUE_)
    pba->Omega0_nfsm += pba->Omega0_cdm2;
  for (n_ncdm=0;n_ncdm<pba->N_ncdm; n_ncdm++) {
  ...
  */
  ...
  if (pba->has_dr == _TRUE_) {
    if (pba->has_dcdm == _TRUE_) {
      ...
      pvecback_integration[pba->index_bi_rho_cdm2] = 0.0;
    }
    else{
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_cdm2",pba->has_cdm2);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_cdm2],pba->has_cdm2,storeidx);
...
int background_derivs(
  ...
  if ((pba->has_dcdm == _TRUE_) && (pba->has_dr == _TRUE_)) {
    ...
    dy[pba->index_bi_rho_dr] = -4.*y[pba->index_bi_rho_dr]+pba->varepsilon*pba->Gamma_dcdm/H*y[pba->index_bi_rho_dcdm];
    dy[pba->index_bi_rho_cdm2] = -3.*y[pba->index_bi_rho_cdm2] +(1-pba->varepsilon)*pba->Gamma_dcdm/H*y[pba->index_bi_rho_dcdm];
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if(pba->has_nudm){
      class_print_species("nu-CDM",nudm);
    ...
    if (pba->has_cdm2 == _TRUE_) {
      class_print_species("Decaying Cold Dark Matter2",cdm2);
      budget_matter+=pba->Omega0_cdm2;
    }
