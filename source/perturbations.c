    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_cdm2],ppt->has_source_delta_cdm2,storeidx);
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_theta_cdm2],ppt->has_source_theta_cdm2,storeidx);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_cdm2",pba->has_cdm2);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_cdm2",pba->has_cdm2);
...
int perturbations_indices(
  ...
  ppt->has_source_delta_cdm2 = _FALSE_;
  ...
  ppt->has_source_theta_cdm2 = _FALSE_;
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_cdm2 == _TRUE_)
          ppt->has_source_delta_cdm2 = _TRUE_;
        if (pba->has_ncdm == _TRUE_)
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_cdm2 == _TRUE_)
          ppt->has_source_theta_cdm2 = _TRUE_;
        if (pba->has_ncdm == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_cdm2, ppt->has_source_delta_cdm2,index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_cdm2, ppt->has_source_theta_cdm2,index_type,1);
...
int perturbations_timesampling_for_sources(
  ...
  if ((ppt->has_cmb == _TRUE_)||(ppt->has_perturbed_recombination == _TRUE_)) {
    ...
    class_call(thermodynamics_at_z(pba,
               ...
               pth->error_message,
               ppt->error_message);
    // printf("pvecthermo[pth->index_th_dmu_urDM]1654=%g,pth->index_th_dmu_urDM=%d\n",pvecthermo[pth->index_th_dmu_urDM],pth->index_th_dmu_urDM);
    class_test(pvecback[pba->index_bg_a]*
               ...
               ppt->error_message,
               "your choice of initial time for computing sources is inappropriate: it corresponds to an earlier time than the one at which the integration of thermodynamical variables started (tau=%g). You should increase either 'start_sources_at_tau_c_over_tau_h=%g' or 'recfast_z_initial'%g,%g,%g\n",
               tau_lower, ppr->start_sources_at_tau_c_over_tau_h, pvecback[pba->index_bg_a],pvecback[pba->index_bg_H],pvecthermo[pth->index_th_dkappa]);
    ...
    class_call(background_at_tau(pba,
               ...
               pba->error_message,
               ppt->error_message);
    // printf("dmu1673=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                   inter_normal,
    ...
    class_test(pvecback[pba->index_bg_a]*
               ...
               ppr->start_sources_at_tau_c_over_tau_h,
               ppt->error_message,
               "your choice of initial time for computing sources is inappropriate:%g,%g,%g it corresponds to a time after recombination. You should decrease 'start_sources_at_tau_c_over_tau_h'\n",pvecback[pba->index_bg_a],pvecback[pba->index_bg_H],pvecthermo[pth->index_th_dkappa]);
    ...
    while (tau_upper - tau_lower > ppr->tol_tau_approx) {
      ...
      class_call(background_at_tau(pba,
                 ...
                 pba->error_message,
                 ppt->error_message);
      // printf("dmu1703=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
      class_call(thermodynamics_at_z(pba,
                                     pth,
                                     1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                     inter_normal,
    ...
    class_call(background_at_tau(pba,
               ...
               pba->error_message,
               ppt->error_message);
    // printf("dmu1756=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                   inter_normal,
  ...
  */
  ...
  while (tau < pba->conformal_age) {
    ...
    class_call(background_at_tau(pba,
               ...
               pba->error_message,
               ppt->error_message);
    // printf("dmu1792=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                   inter_closeby,
  ...
  while (tau < pba->conformal_age) {
    ...
    class_call(background_at_tau(pba,
               ...
               pba->error_message,
               ppt->error_message);
    // printf("dmu1884=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                   inter_closeby,
...
int perturbations_get_k_list(
  ...
  if (ppt->has_scalars == _TRUE_) {
    ...
    else if ((pth->has_cdm2_dr==_TRUE_)&&(pth->n_index_cdm2_dr>=2)){
      class_alloc(ppt->k[ppt->index_md_scalars],
                  ((int)((k_max_cmb[ppt->index_md_scalars]-k_min)/k_rec/MIN(ppr->k_step_super,ppr->k_step_sub))+
                   (int)(MAX(ppr->k_per_decade_for_pk*ppr->cdm2dr_boost_k_per_decade_for_pk*pth->n_index_cdm2_dr,ppr->k_per_decade_for_bao)*log(k_max/k_min)/log(10.))+3)
                  *sizeof(double),ppt->error_message);
    }
    else {
      class_alloc(ppt->k[ppt->index_md_scalars],
    ...
    while (k < k_max) {
      if ((pth->has_idm_dr==_TRUE_)&&(pth->n_index_idm_dr>=2)){
        ...
        k *= pow(10.,1./(ppr->k_per_decade_for_pk*ppr->cdm2dr_boost_k_per_decade_for_pk*pth->n_index_cdm2_dr
                         +(ppr->k_per_decade_for_bao-ppr->k_per_decade_for_pk*ppr->cdm2dr_boost_k_per_decade_for_pk*pth->n_index_cdm2_dr)
                         *(1.-tanh(pow((log(k)-log(ppr->k_bao_center*k_rec))/log(ppr->k_bao_width),4)))));
      }
      else {
        k *= pow(10.,1./(ppr->k_per_decade_for_pk
...
int perturbations_workspace_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_tca_cdm2_dr,pba->has_dr,index_ap,1);
    class_define_index(ppw->index_ap_rsa_dr,pba->has_dr,index_ap,1);
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_dr == _TRUE_)
      ppw->approx[ppw->index_ap_rsa_dr]=(int)rsa_dr_off;
    if (pba->has_dr == _TRUE_)
      ppw->approx[ppw->index_ap_tca_cdm2_dr]=(int)tca_cdm2_dr_off;
    if (pth->has_cdm2_dr == _TRUE_)
      ppw->approx[ppw->index_ap_tca_cdm2_dr]=(int)tca_cdm2_dr_on;
...
int perturbations_solve(
  ...
  // printf("dmu3032=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
  // printf("th_dmu_idm_g3033=%g,pth->index_th_dmu_idm_g=%d,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_idm_g],pth->index_th_dmu_idm_g,pth->th_size);
  class_call(thermodynamics_at_z(pba,
                                 pth,
             ...
             pth->error_message,
             ppt->error_message);
  // printf("dmu3042=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
  // printf("dmu3044=%g\n",pow(1.+z, 2.+pth->n_urDM)*pth->u_urDM_0*3.*pba->H0*pba->H0/8./_PI_/_G_*pba->Omega0_nudm*pow(_c_,4)*_sigma_/1.e11/_eV_/_Mpc_over_m_);
  // printf("th_dmu_idm_g3043=%g,pth->index_th_dmu_idm_g=%d,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_idm_g],pth->index_th_dmu_idm_g,pth->th_size);
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    if (is_early_enough == _TRUE_) {
      // printf("dmu3118=%g\n",pow(1.+z, 2.+pth->n_urDM)*pth->u_urDM_0*3.*pba->H0*pba->H0/8./_PI_/_G_*pba->Omega0_nudm*pow(_c_,4)*_sigma_/1.e11/_eV_/_Mpc_over_m_);
      // printf("th_dmu_idm_g3121=%g,pth->index_th_dmu_idm_g=%d,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_idm_g],pth->index_th_dmu_idm_g,pth->th_size);
      // printf("ppw->pvecthermo values:\n");
      // for (int i = 0; i < pth->th_size; i++) {
      //   printf("ppw->pvecthermo[%d]_before = %g\n", i, ppw->pvecthermo[i]);
      // }
      class_call(thermodynamics_at_z(pba,
                                     pth,
      ...
      // printf("dmu3131=%g\n",pow(1.+z, 2.+pth->n_urDM)*pth->u_urDM_0*3.*pba->H0*pba->H0/8./_PI_/_G_*pba->Omega0_nudm*pow(_c_,4)*_sigma_/1.e11/_eV_/_Mpc_over_m_);
      // printf("ppw->pvecthermo values:\n");
      // for (int i1 = 0; i1 < pth->th_size; i1++) {
      //    printf("ppw->pvecthermo[%d]_after = %g\n", i1, ppw->pvecthermo[i1]);
      //   }
      // printf("th_dmu_idm_g3132=%g,pth->index_th_dmu_idm_g=%d,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_idm_g],pth->index_th_dmu_idm_g,pth->th_size);
...
////////////////////////////////////////////////
  ...
  // if(ppt->perturbations_verbose>0){
  //   printf("is_early_enough=%f\n,account_for_urDM_interactions",is_early_enough,account_for_urDM_interactions);
  // }
  ...
  for (index_interval=0; index_interval<interval_number; index_interval++) {
    ...
    // if(ppt->perturbations_verbose>0){
    //   printf("interval_limit[index_interval]=%g,interval_limit[index_interval+1]=%g",interval_limit[index_interval],interval_limit[index_interval+1]);
    // }
  ...
  //   printf("Integrating from tau = %g to tau = %g\n", interval_limit[index_interval], interval_limit[index_interval + 1]);
  // for (int i = 0; i < ppw->pv->pt_size; i++) {
  //     printf("y[%d] = %g\n,pt_size=%g", i, ppw->pv->y[i],ppw->pv->pt_size);
  // }
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Decaying dark matter2 */
      class_store_columntitle(ppt->scalar_titles, "delta_cdm2", pba->has_cdm2);
      class_store_columntitle(ppt->scalar_titles, "theta_cdm2", pba->has_cdm2);
      /* Decay radiation */
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_dr == _TRUE_){
            if ((interval_approx[index_switch-1][ppw->index_ap_rsa_dr]==(int)rsa_dr_off) &&
                (interval_approx[index_switch][ppw->index_ap_rsa_dr]==(int)rsa_dr_on))
              fprintf(stdout,"Mode k=%e: will switch on decay dark radiation streaming approximation at tau=%e\n",k,interval_limit[index_switch]);
          }

          if (pth->has_cdm2_dr == _TRUE_){
            if ((interval_approx[index_switch-1][ppw->index_ap_tca_cdm2_dr]==(int)tca_cdm2_dr_on) &&
                (interval_approx[index_switch][ppw->index_ap_tca_cdm2_dr]==(int)tca_cdm2_dr_off))
              fprintf(stdout,"Mode k=%e: will switch off dark tight-coupling approximation at tau=%e\n",k,interval_limit[index_switch]);
          }
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    /* cdm2 */
    class_define_index(ppv->index_pt_delta_cdm2,pba->has_cdm2,index_pt,1); /* cdm2 density */
    class_define_index(ppv->index_pt_theta_cdm2,pba->has_cdm2,index_pt,1); /* cdm2 velocity */
    ...
    if (pba->has_dr == _TRUE_){
      if (ppw->approx[ppw->index_ap_rsa_dr]==(int)rsa_dr_off) {
        class_define_index(ppv->index_pt_delta_dr,_TRUE_,index_pt,1); /* density of interacting dark radiation */
        class_define_index(ppv->index_pt_theta_dr,_TRUE_,index_pt,1); /* velocity of interacting dark radiation */
        if (ppt->dr_nature == dr_free_streaming){
          if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off){
            class_define_index(ppv->index_pt_shear_dr,_TRUE_,index_pt,1); /* shear of interacting dark radiation */
            ppv->l_max_dr = ppr->l_max_dr;
            class_define_index(ppv->index_pt_l3_dr,_TRUE_,index_pt,ppv->l_max_dr-2); /* additional momenta in Boltzmann hierarchy (beyond l=0,1,2,3) */
          }
        }
      }
    }
  ...
  if (_scalars_) {
    ...
    if (pba->has_dr == _TRUE_) {
      ...
      /* we don't need interacting dark radiation multipoles
         above l=2 (but they are defined only when rsa_dr
         and tca_cdm2_dr are off) */

      if (ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_off){
        if (ppt->dr_nature == dr_free_streaming){
          if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off){
            for (index_pt=ppv->index_pt_l3_dr; index_pt <= ppv->index_pt_delta_dr+ppv->l_max_dr; index_pt++)
              ppv->used_in_sources[index_pt]=_FALSE_;
          }
        }
      }
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pth->has_cdm2_dr == _TRUE_){
        class_test((pa_old[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off) && (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_on),
                   ppt->error_message,
                   "at tau=%g: the dark tight-coupling approximation can be switched off, not on",tau);
      }
      ...
      if (pba->has_cdm2 == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_cdm2] =
          ppw->pv->y[ppw->pv->index_pt_delta_cdm2];

        ppv->y[ppv->index_pt_theta_cdm2] =
          ppw->pv->y[ppw->pv->index_pt_theta_cdm2];
      }
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_dr == _TRUE_){
          ...
          if (ppw->approx[ppw->index_ap_rsa_dr]==(int)rsa_dr_off){
            ...
            ppv->y[ppv->index_pt_delta_dr] =
              ppw->pv->y[ppw->pv->index_pt_delta_dr];

            ppv->y[ppv->index_pt_theta_dr] =
              ppw->pv->y[ppw->pv->index_pt_theta_dr];

            if (ppt->dr_nature == dr_free_streaming){

              if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off){

                ppv->y[ppv->index_pt_shear_dr] =
                  ppw->pv->y[ppw->pv->index_pt_shear_dr];

                ppv->y[ppv->index_pt_l3_dr] =
                  ppw->pv->y[ppw->pv->index_pt_l3_dr];

                for (l=4; l <= ppv->l_max_dr; l++)
                  ppv->y[ppv->index_pt_delta_dr+l] =
                    ppw->pv->y[ppw->pv->index_pt_delta_dr+l];
              }
            }
          }
        }
      ...
      if (pth->has_idm_dr == _TRUE_) {
        ...
        if ((pa_old[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_on) && (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off)) {
          ...
          if (ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_off) {
            ...
            /* idr is always free streaming if tca_cdm2_dr is on */
            if (ppt->dr_nature == dr_free_streaming){
              ppv->y[ppv->index_pt_shear_dr] = ppw->tca_shear_cdm2_dr;
              ppv->y[ppv->index_pt_l3_dr] = 6./7.*k*ppv->y[ppv->index_pt_shear_dr]/ppw->pvecthermo[pth->index_th_dmu_cdm2_dr]/ppt->alpha_cdm2_dr[1];
            }
          }
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double fracnu,fracg,fracb,fraccdm = 0.,fracidm = 0.,fraccdm2 = 0.;
  double om;
...
//////////////////////////////////////////////////
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm2 == _TRUE_) {
      rho_m += ppw->pvecback[pba->index_bg_rho_cdm2];
    }
    ...
    if (pba->has_idm == _TRUE_){
     ...
     if (pba->has_cdm2 == _TRUE_){
      fraccdm2 =  ppw->pvecback[pba->index_bg_rho_cdm2]/rho_m;
    }
    ...
    */
    ...
    if (pth->has_coupling_urDM == _TRUE_ && ppr->has_urDM_initially == _TRUE_ && ppr->use_urdm_initial_conditions == _TRUE_){
      // printf("dmu5513=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
      class_call(thermodynamics_at_z(pba,
                                     pth,
                                     1./ppw->pvecback[pba->index_bg_a]-1.,  // redshift z=1/a-1
                                     inter_normal,
    ...
    // printf("urDM_thermo_index=%g\n",urDM_thermo_index);
  // printf("dmu5528=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
   //////////////////////////////////////////////////
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_cdm2 == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_cdm2] = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* cdm2 density */
        /* cdm2 velocity velocity vanishes initially in the synchronous gauge */
        ppw->pv->y[ppw->pv->index_pt_theta_cdm2] = ppw->pv->y[ppw->pv->index_pt_theta_g];
...
//////////////////////////////////////////////////
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_nudm == _TRUE_){
	      delta_cdm += ppw->pvecback[pba->index_bg_rho_nudm] * ppw->pv->y[ppw->pv->index_pt_delta_nudm];
        rho_cdm += ppw->pvecback[pba->index_bg_rho_nudm];
      }
      if (pba->has_idm == _TRUE_) {
      ...
      if (pba->has_cdm2 == _TRUE_){
        delta_cdm += ppw->pvecback[pba->index_bg_rho_cdm2] * ppw->pv->y[ppw->pv->index_pt_delta_cdm2];
        rho_cdm += ppw->pvecback[pba->index_bg_rho_cdm2];
      }
      ...
      if (ppt->has_cdm2_dr == _TRUE_ ) {
        delta_tot += rho_m_over_rho_r*fraccdm2*ppw->pv->y[ppw->pv->index_pt_delta_cdm2]/(1.+rho_m_over_rho_r);
        velocity_tot += rho_m_over_rho_r*fraccdm2*ppw->pv->y[ppw->pv->index_pt_theta_cdm2]/(1.+rho_m_over_rho_r);
      }
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_)  || (pba->has_idr == _TRUE_)) {
        ...
        if (pba->has_dr == _TRUE_)
          delta_dr += (-4.*a_prime_over_a + a*pba->varepsilon*pba->Gamma_dcdm*ppw->pvecback[pba->index_bg_rho_dcdm]/ppw->pvecback[pba->index_bg_rho_dr])*alpha;
    ...
    if (pba->has_dr == _TRUE_){
      if (ppw->approx[ppw->index_ap_rsa_dr]==(int)rsa_dr_off) { // TODO: check if needed?
        ppw->pv->y[ppw->pv->index_pt_delta_dr] = delta_ur;
        ppw->pv->y[ppw->pv->index_pt_theta_dr] = theta_ur;
        if (ppt->dr_nature == dr_free_streaming){
          if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off){
            ppw->pv->y[ppw->pv->index_pt_shear_dr] = shear_ur;
            ppw->pv->y[ppw->pv->index_pt_l3_dr] = l3_ur;
          }
        }
      }
    }
...
int perturbations_approximations(
  ...
  double tau_dmu_idm_g = 0., tau_dmu_idm_dr = 0., tau_dmu_cdm2_dr = 0.;
  ...
  if (_scalars_) {
    ...
    // printf("dmu6235=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./ppw->pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
    ...
    if (pth->has_cdm2_dr == _TRUE_){
      if (ppw->pvecthermo[pth->index_th_dmu_cdm2_dr] == 0.){
        ppw->approx[ppw->index_ap_tca_cdm2_dr] = (int)tca_cdm2_dr_off;
      }
      else{
        tau_dmu_cdm2_dr = 1./ppw->pvecthermo[pth->index_th_dmu_cdm2_dr];

        class_test(tau_dmu_cdm2_dr < 0.,
                   ppt->error_message,
                   "negative tau_cdm2_dr=1/dmu_cdm2_dr=%e at z=%e, conformal time=%e.\n",
                   tau_dmu_cdm2_dr,
                   1./ppw->pvecback[pba->index_bg_a]-1.,
                   tau);

        if ((tau_dmu_cdm2_dr / tau_h < ppr->cdm2_dr_tight_coupling_trigger_tau_c_over_tau_h) &&
            (tau_dmu_cdm2_dr / tau_k < ppr->cdm2_dr_tight_coupling_trigger_tau_c_over_tau_k) &&
            (pth->n_index_cdm2_dr>=2) && (ppt->idr_nature == idr_free_streaming)) {
          ppw->approx[ppw->index_ap_tca_cdm2_dr] = (int)tca_cdm2_dr_on;
        }
        else{
          ppw->approx[ppw->index_ap_tca_cdm2_dr] = (int)tca_cdm2_dr_off;
    ...
    if (pba->has_dr == _TRUE_){
      ...
      if ((tau/tau_k > ppr->dr_streaming_trigger_tau_over_tau_k) &&
          (tau > pth->tau_dr_free_streaming) &&
          (pth->has_cdm2_dr == _FALSE_ || pth->n_index_cdm2_dr >= 2) &&
          (ppr->dr_streaming_approximation != rsa_idr_none)){

        ppw->approx[ppw->index_ap_rsa_dr] = (int)rsa_dr_on;
      }
      else{
        ppw->approx[ppw->index_ap_rsa_dr] = (int)rsa_dr_off;
      }
  ...
  if (_tensors_) {
    ...
    /** - --> (a) evaluate thermodynamical quantities with thermodynamics_at_z() */
    // printf("dmu6391=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
    class_call(thermodynamics_at_z(pba,
                                   pth,
                                   1./ppw->pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                   (interpolation_method)ppw->inter_mode,
...
int perturbations_timescale(
  ...
  if ((ppt->has_scalars == _TRUE_) && (pppaw->index_md == ppt->index_md_scalars)) {
    ...
    *timescale = tau_h;
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
      // printf("dmu6527=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
      class_call(thermodynamics_at_z(pba,
                                     pth,
                                     1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                     (interpolation_method)ppw->inter_mode,
  ...
  if ((ppt->has_vectors == _TRUE_) && (pppaw->index_md == ppt->index_md_vectors)) {
    ...
    *timescale = MIN(tau_h,tau_k);
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
      // printf("dmu6558=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
      class_call(thermodynamics_at_z(pba,
                                     pth,
                                     1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                     (interpolation_method)ppw->inter_mode,
  ...
  if ((ppt->has_tensors == _TRUE_) && (pppaw->index_md == ppt->index_md_tensors)) {
    ...
    *timescale = MIN(tau_h,tau_k);
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
      // printf("dmu6588=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
      class_call(thermodynamics_at_z(pba,
                                     pth,
                                     1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                     (interpolation_method)ppw->inter_mode,
...
int perturbations_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if ((pba->has_dr==_TRUE_)&&(ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_on)) {

        class_call(perturbations_rsa_dr_delta_and_theta(ppr,pba,pth,ppt,k,y,a_prime_over_a,ppw->pvecthermo,ppw,ppt->error_message),
                   ppt->error_message,
                   ppt->error_message);

        ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_dr]*ppw->rsa_theta_dr;
      }
      ...
      if (pth->has_idm_dr == _TRUE_) {
       ...
       if (pth->has_cdm2_dr == _TRUE_) {
        if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_on){

          shear_dr = 0.5*8./15./ppw->pvecthermo[pth->index_th_dmu_cdm2_dr]/ppt->alpha_cdm2_dr[0]*(y[ppw->pv->index_pt_theta_dr]+k2*ppw->pvecmetric[ppw->index_mt_alpha]);

          ppw->rho_plus_p_shear += 4./3.*ppw->pvecback[pba->index_bg_rho_dr]*shear_dr;
        }
      }
...
int perturbations_total_stress_energy(
  ...
  double p_phy,epsilon_phy,epsilon_now,ratio;
   double C_nudm, C_nudm_int, divisor_int, C_nudm_int_theta=0, C_nudm_int_psi1=0, divisor_int_mean=0;
  double w_fld,dw_over_da_fld,integral_fld;
  double gwncdm;
  ...
  if (_scalars_) {
    ...
    if (pba->has_dr == _TRUE_) {
      if (ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_off) {
        delta_dr = y[ppw->pv->index_pt_delta_dr];
        theta_dr = y[ppw->pv->index_pt_theta_dr];

        if (ppt->dr_nature == dr_free_streaming){
          if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_on){
            if (ppt->gauge == newtonian)
              shear_dr = 0.5 *(8./15./ppw->pvecthermo[pth->index_th_dmu_cdm2_dr]/ppt->alpha_cdm2_dr[0]*(y[ppw->pv->index_pt_theta_dr])) ;
            else
              shear_dr = 0.; /* this is set in perturbations_einstein, so here it's set to 0 */
          }
          else{
            shear_dr = y[ppw->pv->index_pt_shear_dr];
          }
        }
      }
      else{
    ...
    if (pba->has_cdm2 == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_cdm2]*y[ppw->pv->index_pt_delta_cdm2];
      ppw->rho_plus_p_theta += ppw->pvecback[pba->index_bg_rho_cdm2]*y[ppw->pv->index_pt_theta_cdm2];

      ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_cdm2];

      if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_cdm2]*y[ppw->pv->index_pt_delta_cdm2]; // contribution to delta rho_matter
        rho_m += ppw->pvecback[pba->index_bg_rho_cdm2];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_rho_cdm2]*y[ppw->pv->index_pt_theta_cdm2]; // contribution to [(rho+p)theta]_matter
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_cdm2];
      }
    }
    /* ultra-relativistic decay radiation */
    ...
    if (pba->has_dr == _TRUE_ && pba->has_cdm2_dr == _TRUE_){
      ...
      if (ppt->dr_nature==dr_free_streaming)
        ppw->rho_plus_p_shear += 4./3.*ppw->pvecback[pba->index_bg_rho_dr]*shear_dr;
      ppw->delta_p += 1./3. * ppw->pvecback[pba->index_bg_rho_dr]*delta_dr;
      ppw->rho_plus_p_tot += 4./3. * ppw->pvecback[pba->index_bg_rho_dr];
    }
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        ...
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          for(index_q=0; index_q < 11; index_q ++){
            q = pba->q_ncdm[n_ncdm][index_q];
            q2 = q*q;
            p_phy = q/a;
            epsilon = sqrt(q*q+a2*pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
            epsilon_phy = sqrt(p_phy*p_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
            epsilon_now = sqrt(q*q+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
            ratio = (epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm])/(epsilon_now*epsilon_now+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
            C_nudm = ppw->pvecback[pba->index_bg_A_nudm1+n_ncdm]*q2/epsilon/epsilon;
            C_nudm_int_theta += 2*q2*q*pba->w_ncdm[n_ncdm][index_q]*ratio*(1+p_phy*p_phy/3./(epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]))*q2/epsilon/epsilon/3.* epsilon/q *pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
            C_nudm_int_psi1 +=2*q2*pba->w_ncdm[n_ncdm][index_q]*q2/epsilon/epsilon*ratio*(1+p_phy*p_phy/3./(epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]));
            divisor_int_mean +=q2*pba->w_ncdm[n_ncdm][index_q];
            divisor_int +=q2*epsilon*pba->w_ncdm[n_ncdm][index_q]+1/3*q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q];
          ...
          if ((ppt->has_source_delta_ncdm == _TRUE_) || (ppt->has_source_theta_ncdm == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            if (pba->has_ncdm_dm_interactions == _TRUE_) {
              ...
              *(y[ppw->pv->index_pt_theta_nudm]*(C_nudm_int_theta/divisor_int) + y[idx+1] * (C_nudm_int_psi1/divisor_int_mean));
              // ADD this to nudm euler equation (do not subtract, sign is absorbed here)
        ...
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
            ...
            if (pba->has_ncdm_dm_interactions == _TRUE_) {
              ...
              C_nudm_int += q2*q*pba->w_ncdm[n_ncdm][index_q]*C_nudm*2*ratio*(1+p_phy*p_phy/3./(epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]))*(y[idx+1]
          ...
          if ((ppt->has_source_delta_ncdm == _TRUE_) || (ppt->has_source_theta_ncdm == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            if (pba->has_ncdm_dm_interactions == _TRUE_) {
              ppw->nudm_interaction_term[n_ncdm] =
             k*(ppw->pvecback[pba->index_bg_rho_ncdm1+n_ncdm]+ppw->pvecback[pba->index_bg_p_ncdm1+n_ncdm])/ppw->pvecback[pba->index_bg_rho_nudm]
              * C_nudm_int/divisor_int; // ADD this to nudm euler equation (do not subtract, sign is absorbed here)
...
int perturbations_sources(
  ...
  double theta_idm = 0., theta_idm_prime = 0., theta_cdm2 = 0., theta_cdm2_prime = 0.;
  ...
  // printf("dmu7667=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
  class_call(thermodynamics_at_z(pba,
                                 pth,
                                 z,  /* redshift z=1/a-1 */
                                 inter_closeby,
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_cdm2 == _TRUE_) {
      _set_source_(ppt->index_tp_delta_cdm2) = y[ppw->pv->index_pt_delta_cdm2]
        + (3.*a_prime_over_a-a*(1-pba->varepsilon)*pba->Gamma_dcdm)*theta_over_k2; // N-body gauge correction;
    } 
    ...
    if (ppt->has_source_theta_cdm2 == _TRUE_) {
      _set_source_(ppt->index_tp_theta_cdm2) = y[ppw->pv->index_pt_theta_cdm2]
        + theta_shift; // N-body gauge correction
    }
...
int perturbations_print_variables(double tau,
  ...
  double delta_cdm2=0.,theta_cdm2=0.;
  ...
  class_call(background_at_tau(pba,
             ...
             pba->error_message,
             error_message);
  // printf("dmu8352=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
  class_call(thermodynamics_at_z(pba,
                                 pth,
  ...
  if (_scalars_) {
    ...
    if (pba->has_dr == _TRUE_) {
      if (ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_off) {
        ...
        if (ppt->dr_nature == dr_free_streaming){
          if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_on){
            shear_dr = ppw->tca_shear_cdm2_dr;
          }
          else{
            shear_dr = y[ppw->pv->index_pt_shear_dr];
          }
        ...
        delta_dr = ppw->rsa_delta_dr;
        theta_dr = ppw->rsa_theta_dr;
        shear_dr = 0.;
      }
    }
    ...
    if (pba->has_cdm2 == _TRUE_) {
      delta_cdm2 = y[ppw->pv->index_pt_delta_cdm2];
      theta_cdm2 = y[ppw->pv->index_pt_theta_cdm2];
    }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_dr == _TRUE_) {
        delta_dr += (-4.*a*H+a*pba->varepsilon*pba->Gamma_dcdm*pvecback[pba->index_bg_rho_dcdm]/pvecback[pba->index_bg_rho_dr])*alpha;
      ...
      if (pba->has_cdm2 == _TRUE_) {
        delta_cdm2 += alpha*(+a*(1-pba->varepsilon)*pba->Gamma_dcdm-3.*a*H);
        theta_cdm2 += k*k*alpha;
      }
    ...
    class_store_double(dataptr, delta_cdm2, pba->has_cdm2, storeidx);
    class_store_double(dataptr, theta_cdm2, pba->has_cdm2, storeidx);
    /* Decay radiation */
...
int perturbations_derivs(double tau,
  ...
  double c2_idm=0., S_idm_g=0., c2_cdm2;
  double dmu_idm_g = 0., photon_scattering_rate;
  double S_idm_dr=0., dmu_idm_dr=0., dmu_idr=0., tca_slip_idm_dr=0., S_cdm2_dr=0., dmu_cdm2_dr=0., dmu_dr=0., tca_slip_cdm2_dr=0.;
  ...
  int index_q,n_ncdm,idx;
  double q,q2,epsilon,dlnf0_dlnq,qk_div_epsilon,p_phy,epsilon_phy,epsilon_now,ratio;
  double rho_ncdm_bg,p_ncdm_bg,pseudo_p_ncdm,w_ncdm,ca2_ncdm,ceff2_ncdm=0.,cvis2_ncdm=0.;
  double C_nudm = 0, C_nudm_int_psi2=0, divisor_int_mean=0;  //allocate to 0, set to value if there is an interaction.
  double sigma_term = 0;
  ...
  class_call(background_at_tau(pba,
             ...
             pba->error_message,
             error_message);
  // printf("dmu9000=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);
  class_call(thermodynamics_at_z(pba,
                                 pth,
                                 1./pvecback[pba->index_bg_a]-1.,  /* redshift z=1/a-1 */
                                 inter_closeby,
             ...
             pth->error_message,
             error_message);
  // printf("dmu9010=%g,pba->H0=%g,pth->n_urDM=%g,size=%d\n",ppw->pvecthermo[pth->index_th_dmu_urDM],pba->H0,pth->n_urDM,pth->th_size);        
  // printf("pvecthermo[pth->index_th_dmu_urDM]9010=%g,pth->index_th_dmu_urDM=%d\n",pvecthermo[pth->index_th_dmu_urDM],pth->index_th_dmu_urDM);
  ...
  if (pba->has_cdm2 == _TRUE_) {
    ...
    if (ppt->has_cdm2_soundspeed == _TRUE_) {
      c2_cdm2 = pvecthermo[pth->index_th_c2_cdm2];
    }
    else {
      c2_cdm2 = 0.;
    }  
    if (pth->has_cdm2_dr == _TRUE_){
      S_cdm2_dr = 4./3. * pvecback[pba->index_bg_rho_dr]/ pvecback[pba->index_bg_rho_cdm2];
      dmu_cdm2_dr = pvecthermo[pth->index_th_dmu_cdm2_dr];
      dmu_dr = pth->b_dr/pth->a_cdm2_dr*pba->Omega0_dr/pba->Omega0_cdm2*dmu_cdm2_dr;
    }
  ...
  if (_scalars_) {
   ...
   if (pba->has_dr == _TRUE_){
      if (ppw->approx[ppw->index_ap_rsa_dr] == (int)rsa_dr_off){
        delta_dr = y[pv->index_pt_delta_dr];
        theta_dr = y[pv->index_pt_theta_dr];
      }
    } 
    ...
    if (pba->has_cdm2 == _TRUE_){
      delta_cdm2 = y[pv->index_pt_delta_cdm2];
      theta_cdm2 = y[pv->index_pt_theta_cdm2];
      ppw->theta_cdm2 = theta_cdm2;
    }
  ...
  //  printf("derivs einstein nudm done\n"); //debug
    ...
    if ((pba->has_dcdm == _TRUE_)&&(pba->has_dr == _TRUE_)) {
      ...
      dy[pv->index_pt_delta_cdm2] = -(y[pv->index_pt_theta_cdm2]+metric_continuity)
        + a *(1-pba->varepsilon)* pba->Gamma_dcdm / k2 * metric_euler; /* dcdm density */

      dy[pv->index_pt_theta_cdm2] = - a_prime_over_a*y[pv->index_pt_theta_cdm2] + metric_euler; 
      /* f = rho_dr*a^4/rho_crit_today. In CLASS density units
         rho_crit_today = H0^2.
      */
      ...
      if (pba->has_cdm2_dr == _TRUE_) {
        ...
        dy[pv->index_pt_delta_dr] = -4./3.*(theta_dr + metric_continuity)+
          a * pba->varepsilon * pba->Gamma_dcdm * pvecback[pba->index_bg_rho_dcdm]/pvecback[pba->index_bg_rho_dr]*(y[pv->index_pt_delta_dcdm]- y[pv->index_pt_delta_dr]+metric_euler/k2);
        // if (ppw->approx[ppw->index_ap_tca_cdm2_dr] == (int)tca_cdm2_dr_off) {

          /** - ----> idr velocity */
        if (ppt->dr_nature == dr_free_streaming){
           dy[pv->index_pt_theta_dr] = k2*(y[pv->index_pt_delta_dr]/4.-s2_squared*y[pv->index_pt_shear_dr]) + metric_euler 
            -a * pba->varepsilon * pba->Gamma_dcdm * 3 * pvecback[pba->index_bg_rho_dcdm]/4./pvecback[pba->index_bg_rho_dr]
            *(4*y[pv->index_pt_theta_dr]/3-y[pv->index_pt_theta_dcdm]);
          }
        else{
            dy[pv->index_pt_theta_dr] = k2*(y[pv->index_pt_delta_dr]/4.-s2_squared*y[pv->index_pt_shear_dr]) + metric_euler 
            -a * pba->varepsilon * pba->Gamma_dcdm * 3 * pvecback[pba->index_bg_rho_dcdm]/4./pvecback[pba->index_bg_rho_dr]
            *(4*y[pv->index_pt_theta_dr]/3-y[pv->index_pt_theta_dcdm]);
            dy[pv->index_pt_theta_dr] += dmu_cdm2_dr*(y[pv->index_pt_theta_cdm2]-y[pv->index_pt_theta_dr]);
          }
        ...
        if (ppt->dr_nature == dr_free_streaming){
            ...
            /** - ----> exact idr shear */
            l = 2;
            dy[pv->index_pt_shear_dr] = 0.5*(8./15.*(y[pv->index_pt_theta_dr]+metric_shear)-3./5.*k*s_l[3]/s_l[2]*y[pv->index_pt_shear_dr+1]);
            dy[pv->index_pt_shear_dr]-= (ppt->alpha_cdm2_dr[l-2]*dmu_cdm2_dr + ppt->beta_dr[l-2]*dmu_dr)*y[pv->index_pt_shear_dr];

            /** - ----> exact idr l=3 */
            l = 3;
            dy[pv->index_pt_l3_dr] = k/(2.*l+1.)*(l*2.*s_l[l]*s_l[2]*y[pv->index_pt_shear_dr]-(l+1.)*s_l[l+1]*y[pv->index_pt_l3_dr+1]); 
            dy[pv->index_pt_l3_dr]-= (ppt->alpha_cdm2_dr[l-2]*dmu_cdm2_dr + ppt->beta_dr[l-2]*dmu_dr)*y[pv->index_pt_l3_dr];

            /** - ----> exact idr l>3 */
            for (l = 4; l < pv->l_max_dr; l++) {
              dy[pv->index_pt_delta_dr+l] = k/(2.*l+1)*(l*s_l[l]*y[pv->index_pt_delta_dr+l-1]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_dr+l+1]);
              dy[pv->index_pt_delta_dr+l]-= (ppt->alpha_cdm2_dr[l-2]*dmu_cdm2_dr + ppt->beta_dr[l-2]*dmu_dr)*y[pv->index_pt_delta_dr+l];
            }

            /** - ----> exact idr lmax_dr */
            l = pv->l_max_dr;
            dy[pv->index_pt_delta_dr+l] = k*(s_l[l]*y[pv->index_pt_delta_dr+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_dr+l]);
            dy[pv->index_pt_delta_dr+l]-= (ppt->alpha_cdm2_dr[l-2]*dmu_cdm2_dr + ppt->beta_dr[l-2]*dmu_dr)*y[pv->index_pt_delta_dr+l];
         }
        // }
        // else{
        //   // dy[pv->index_pt_theta_dr] = ppw->theta_cdm2_prime - tca_slip_cdm2_dr;
    ...
    if (pba->has_ur == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        if(pth->has_coupling_urDM==_TRUE_)
              ...
              //  printf("dy[pv->index_pt_theta_ur]=%g,pvecthermo[pth->index_th_dmu_urDM]=%g,y[pv->index_pt_theta_nudm]=%g\n",dy[pv->index_pt_theta_ur],pvecthermo[pth->index_th_dmu_urDM],y[pv->index_pt_theta_nudm]);
        if (ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
          ...
          if(pth->has_coupling_urDM==_TRUE_)
            ...
            // printf("dy[pv->index_pt_shear_ur]=%g\n",dy[pv->index_pt_shear_ur]);
          /** - -----> exact ur l=3 */
          l = 3;
          ...
          if(pth->has_coupling_urDM==_TRUE_)
            ...
            // printf("dy[pv->index_pt_delta_ur+%g]=%g,pv->l_max_ur=%g\n",l, dy[pv->index_pt_delta_ur+l],pv->l_max_ur);
          ...
          if(pth->has_coupling_urDM==_TRUE_)
            dy[pv->index_pt_delta_ur+l] -= ppt->alpha_urDM[l-1]*pvecthermo[pth->index_th_dmu_urDM]*y[pv->index_pt_delta_ur+l]; 
            // printf("dy[pv->index_pt_delta_ur+%g]=%g,pv->l_max_ur=%g\n",l, dy[pv->index_pt_delta_ur+l],pv->l_max_ur);
            // printf("dy[pv->index_pt_delta_ur+l]=%g,ppt->alpha_urDM[l-1]=%g,pvecthermo[pth->index_th_dmu_urDM]=%g,y[pv->index_pt_delta_ur+l]=%g\n",dy[pv->index_pt_delta_ur+l],ppt->alpha_urDM[l-1],pvecthermo[pth->index_th_dmu_urDM],y[pv->index_pt_delta_ur+l]); 
      ...
      if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
        ...
        for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
         ...
         for(index_q=0; index_q < 11; index_q ++){
            ...
            ratio = (epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm])/(epsilon_now*epsilon_now+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
            C_nudm = ppw->pvecback[pba->index_bg_A_nudm1+n_ncdm]*q2/epsilon/epsilon;
            C_nudm_int_psi2 +=2*q2*pba->w_ncdm[n_ncdm][index_q]*C_nudm*ratio;
            divisor_int_mean +=q2*pba->w_ncdm[n_ncdm][index_q];
          ...
          if (pba->has_ncdm_dm_interactions == _TRUE_){
            ...
            C_nudm = ppw->pvecback[pba->index_bg_A_nudm1+n_ncdm]*q*q/epsilon/epsilon;
            dy[idx+1] -= pvecback[pba->index_bg_rho_nudm]/(rho_ncdm_bg+p_ncdm_bg) *ppw->nudm_interaction_term[n_ncdm]; //
     
             
             sigma_term = y[idx+2]*C_nudm_int_psi2/divisor_int_mean;
          }
          //printf("sigma_term=%g\n",sigma_term); //debug
          ...
          if (ppr->ncdm_fluid_approximation == ncdmfa_mb) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ncdm-pseudo_p_ncdm/p_ncdm_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_ncdm/(1.0+w_ncdm)*s_l[2]*(y[idx+1]+metric_shear) - sigma_term;
          ...
          if (ppr->ncdm_fluid_approximation == ncdmfa_CLASS) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_ncdm-pseudo_p_ncdm/p_ncdm_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_ncdm/(1.0+w_ncdm)*s_l[2]*(y[idx+1]+metric_ufa_class) - sigma_term;
        ...
        for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
          ...
          for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
            ...
            dy[idx+1] = qk_div_epsilon/3.0*(y[idx] - 2*s_l[2]*y[idx+2])
              ...
              if (pba->has_ncdm_dm_interactions == _TRUE_) {
               // dy[idx+1] += -C_nudm*(y[idx+1] + y[pv->index_pt_theta_nudm]/3./k * epsilon/q * dlnf0_dlnq); 
                dy[idx+1] += -C_nudm*ratio*2*(1+p_phy*p_phy/3./(epsilon_phy*epsilon_phy+pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]))*(y[idx+1] + y[pv->index_pt_theta_nudm]/3./k * epsilon/q * dlnf0_dlnq);
                 // nudm interaction term
                 //printf("dy[idx+1]=%g\n",dy[idx+1]);
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_) {
      ...
      dy[pv->index_pt_delta_ur+l] =
        k*(s_l[l]*y[pv->index_pt_delta_ur+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_ur+l]);
      printf("s_l[l]=%g,y[pv->index_pt_delta_ur+l-1]=%g,\n",s_l[l],y[pv->index_pt_delta_ur+l-1]);
    }
