...
int input_shooting(struct file_content * pfc,
  ...
  char * const unknown_namestrings[] = {"h",                        /* unknown param for target '100*theta_s' */
                                        ...
                                        "Omega_dcdmdrcdm",             /* unknown param for target 'Omega_ini_dcdm' */
                                        "omega_dcdmdrcdm"};             /* unknown param for target 'omega_ini_dcdm' */
...
int input_get_guess(double *xguess,
  ...
  int i;
  double Omega_M, a_decay, gamma, varepsilon, Omega0_dcdmdrcdm=1.0;
  int index_guess;
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s_100:
      ...
      break;
    case Omega_dcdmdrcdm:
      Omega_M = ba.Omega0_cdm+ba.Omega0_idm+ba.Omega0_dcdmdrcdm+ba.Omega0_b;
      /* *
    ...
    case Omega_scf:
      ...
      break;
    case omega_ini_dcdm:
      Omega0_dcdmdrcdm = 1./(ba.h*ba.h);
    case Omega_ini_dcdm:
      /* This works since correspondence is Omega_ini_dcdm -> Omega_dcdmdr and
         omega_ini_dcdm -> omega_dcdmdr */
      Omega0_dcdmdrcdm *=pfzw->target_value[index_guess];
      Omega_M = ba.Omega0_cdm+ba.Omega0_idm+Omega0_dcdmdrcdm+ba.Omega0_b;
      gamma = ba.Gamma_dcdm/ba.H0;
      if (gamma < 1)
        a_decay = 1.0;
      else
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  int i;
  double rho_dcdm_today, rho_dr_today, rho_cdm2_today;
  struct fzerofun_workspace * pfzw;
  int input_verbose;
  int flag;
  int param;
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_dcdmdrcdm:
      ...
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      if (ba.has_cdm2 == _TRUE_)
        rho_cdm2_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_cdm2];
      else
        rho_cdm2_today = 0.;
      output[i] = (rho_dcdm_today+rho_dr_today+rho_cdm2_today)/(ba.H0*ba.H0)-pfzw->target_value[i];
      break;
    case omega_dcdmdrcdm:
      rho_dcdm_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
      if (ba.has_dr == _TRUE_)
      ...
      if (ba.has_cdm2 == _TRUE_)
        ...
        rho_cdm2_today = 0.;
      output[i] = (rho_dcdm_today+rho_dr_today+rho_cdm2_today)/(ba.H0*ba.H0)-pfzw->target_value[i]/ba.h/ba.h;
      break;
    case Omega_scf:
    ...
    case omega_ini_dcdm:
      ...
      if (ba.has_cdm2 == _TRUE_)
        ...
        rho_cdm2_today = 0.;  
      output[i] = -(rho_dcdm_today+rho_dr_today+rho_cdm2_today)/(ba.H0*ba.H0)+ba.Omega0_dcdmdrcdm;
      break;
    case sigma8:
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (N_ncdm > 0){
    ...
    //  class_call(parser_read_double(pfc,"m_ncdm_tot",&param2,&flag3,errmsg),
    //          errmsg,
    //          errmsg);
  ...
  class_call(parser_read_double(pfc,"Omega_dcdmdrcdm",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_dcdmdrcdm",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "You can only enter one of 'Omega_dcdmdrcdm' or 'omega_dcdmdrcdm'.");
  ...
  if (pba->Omega0_dcdmdrcdm > 0 || (pba->Omega_ini_dcdm > 0.)) {
  ...
  class_call(parser_read_double(pfc,"u_urDM_0",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "You can only enter one of 'u_urDM_0' or 'logu_urDM_0'.");
  /* Complete set of parameters */
  if (flag1 == _TRUE_){
    pth->u_urDM_0 = param1;
  }
...
/** this is a bit special: if nu-DM scattering is enabled initialize the list of hihger-order
  ...
  Omega_tot += pba->Omega0_dcdmdrcdm;
...
int input_default_params(struct background *pba,
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdrcdm - pba->Omega0_idr -pba->Omega0_idm - pba->Omega0_nudm;
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
